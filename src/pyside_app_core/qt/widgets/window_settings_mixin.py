...
class WindowSettingsMixin(ObjectNameMixin, SettingsMixin):
    def __init__(
        self, parent: QObject | None = None, *args: object, **kwargs: object
    ) -> None:
    ...
    def _restore_state(self) -> None:
        cast(QWidget, self).restoreGeometry(
            cast(QByteArray, self._settings.value(f"{self.obj_name}_geometry"))
        )

        if isinstance(self, QMainWindow):
            self.restoreState(
                cast(QByteArray, self._settings.value(f"{self.obj_name}_window_state"))
            )
